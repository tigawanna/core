...
test('checkWebAppManifest - noManifest', async () => {
  await runCheckTouchIconTitleTest('<title>Hey</title>', {
    messages: [
      {
        status: CheckerStatus.Error,
...
test('checkWebAppManifest - noManifestHref', async () => {
  await runCheckTouchIconTitleTest('<link rel="manifest" />', {
    messages: [
      {
        status: CheckerStatus.Error,
...
test('checkWebAppManifest - manifest404', async () => {
  await runCheckTouchIconTitleTest('<link rel="manifest" href="not-found.json" />', {
    messages: [
      {
        status: CheckerStatus.Error,
...
test('checkWebAppManifest - manifestCannotGet', async () => {
  await runCheckTouchIconTitleTest(
    '<link rel="manifest" href="/error.json" />',
    {
      messages: [
        {
          status: CheckerStatus.Error,
