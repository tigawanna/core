...
program.name('realfavicon').description('Generate and check favicon with RealFaviconGenerator').version(p.version);
...
program
  ...
  .option(
    '-s, --screen <screen>',
    'Screen where the report is displayed, can be cli or realfavicon (ie. the browser)',
    'realfavicon',
  )
...
program
  ...
  .argument(
    '<settings>',
    `Path of the favicon settings file, which can be created via ${RealFaviconGeneratorBaseUrl}favicon/command-line`,
  )
