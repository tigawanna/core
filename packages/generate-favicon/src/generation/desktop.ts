...
export const generateDesktopFaviconHtml = (
  faviconPath: string,
...
export const generateDesktopFaviconFiles = async (
  masterIcon: MasterIcon,
  settings: DesktopIconSettings,
  imageAdapter: ImageAdapter,
  skipMetadata = false,
): Promise<FaviconFiles> => {
  const transformedRegularSvg = transformSvg(masterIcon.icon, settings.regularIconTransformation, imageAdapter, 128);
  ...
  const pics = await Promise.all(
    IcoFaviconSizes.map(async size => {
